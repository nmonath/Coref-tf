...
#Each document is split into a set of subdocuments with length set to window_size
flags.DEFINE_integer("num_window", 5, "The max number of windows for one document")
#This is used for fitting a document into fix shape for TF computation. 
#If a document is longer than num_window*window_size, the exceeding part will be abandoned. 
#This only affects training and does not affect test, since the all 
